...
const Readings = ({ manga, readings }: ReadingsProps) => {
  ...
  const prepareForm = (isNew: boolean, reading: { title: string, chapter: number }) => {
  ...
  return (
    <div className="tab-pane fade show active" id="pills-reading" role="tabpanel" aria-labelledby="pills-reading-tab">
      ...
      <button type='button' className='btn btn-primary mb-3' data-bs-toggle='modal' data-bs-target='#ReadingForm' onClick={() => prepareForm(true, { title: '', chapter: 0})}>Add New Manga</button>
                  ...
                  return (
                    <tr key={reading.manga}>
                      ...
                      <td className='profile-actions'>
                        <button type='button' className='btn btn-primary btn-sm' data-bs-toggle='modal' data-bs-target='#ReadingForm' onClick={() => prepareForm(false, { title: data.title, chapter: data.chapter })}>Edit</button>{' '}<a href='#!' className='btn btn-sm btn-danger' onClick={() => onClick(reading._id)}>Delete</a></td>
                    </tr>
                  )
